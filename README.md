...
- ```itps/```
  ...
  + ```posre_protein.itp``` - xxx
  + ```water.em.itp``` - 
  + ```water.md.itp``` - 
- ```gen_conf/```
  + ```create_prot_gro.py``` - Script to construct the initial gro file of the protein from its sequence 
  + ```+gen_prot_em.itp.py``` - Script to contruct the itp file for the protein
  + ```seq.txt``` - Input for both ```create_prot_gro.py``` and ```+gen_prot_em.itp.py``` 
- ```em_md/```  - Folder with all tprs
